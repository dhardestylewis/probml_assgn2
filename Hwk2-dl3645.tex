...
\textbf{Bernoulli representation.} Define
\[
p_{ij} := \Pr(y_{ij}=1 \mid \theta_i, \alpha_j, \beta_j) = \Phi\!\big(\alpha_j(\theta_i - \beta_j)\big).
\]
Then, for a single observed vote $y_{ij} \in \{0,1\}$:
\begin{itemize}
\item If $y_{ij}=1$,
\[
\Pr(y_{ij} \mid \theta_i, \alpha_j, \beta_j) = p_{ij}.
\]
\item If $y_{ij}=0$,
\[
\Pr(y_{ij} \mid \theta_i, \alpha_j, \beta_j) = 1 - p_{ij}.
\]
\end{itemize}
...
\]
Check both possible values of $y_{ij}$:
\begin{itemize}
\item If $y_{ij}=1$:
\[
p_{ij}^{\,1} (1-p_{ij})^{\,0} = p_{ij} \cdot 1 = p_{ij},
\]
which matches $\Pr(y_{ij}=1 \mid \theta_i, \alpha_j, \beta_j)$.

\item If $y_{ij}=0$:
\[
p_{ij}^{\,0} (1-p_{ij})^{\,1} = 1 \cdot (1-p_{ij}) = 1 - p_{ij},
\]
which matches $\Pr(y_{ij}=0 \mid \theta_i, \alpha_j, \beta_j)$.
\end{itemize}

Therefore, the two cases above are exactly equivalent to:
\[
\Pr(y_{ij} \mid \theta_i, \alpha_j, \beta_j) = p_{ij}^{\,y_{ij}} (1-p_{ij})^{\,1-y_{ij}}.
\]
Substituting $p_{ij} = \Phi\big(\alpha_j(\theta_i - \beta_j)\big)$:
\[
\Pr(y_{ij} \mid \theta_i, \alpha_j, \beta_j) = [\Phi\!\big(\alpha_j(\theta_i - \beta_j)\big)]^{y_{ij}} [1-\Phi\!\big(\alpha_j(\theta_i - \beta_j)\big)]^{1-y_{ij}}.
\]

\textbf{Joint likelihood.} Define the index set of non-missing votes:
\[
O := \{ (i,j) : y_{ij} \in \{0,1\} \}.
\]
Under the usual assumption that votes are conditionally independent given $\{\theta_i\}$ and $\{\alpha_j, \beta_j\}$,
\[
p(\{y_{ij}\}_{(i,j)\in O} \mid \{\theta_i\}, \{\alpha_j, \beta_j\}) = \prod_{(i,j)\in O} \Pr(y_{ij} \mid \theta_i, \alpha_j, \beta_j).
\]
Substituting the single-vote Bernoulli form:
\[
...
\begin{figure}[h]
\centering
\includegraphics[width=0.6\textwidth]{ideal_point_plate_fixed.png}
\end{figure}
...
\begin{figure}[h]
\centering
\includegraphics[width=0.8\textwidth]{ideology_line_threshold.png}
\end{figure}
