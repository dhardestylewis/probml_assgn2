...
\begin{table}[t]
    ...
    \begin{tabular}{lc}
        ...
        Metric & Value \\
        \midrule
        Avg.\ log $p(y \mid x)$ (log-price, 20\% hold-out) & $-1.15$ \\
        RMSE (log-price) & $1.84$ \\
        MAE (log-price) & $0.97$ \\
        RMSE (price in \$) & $2.46 \times 10^7$ \\
        MAE (price in \$) & $2.18 \times 10^6$ \\
        Median $|y - \hat{y}|$ (log-price) & $0.23$ \\
        95th pct.\ $|y - \hat{y}|$ (log-price) & $4.09$ \\
        \bottomrule
    \end{tabular}
    \caption{Global posterior predictive metrics for the SemiSupMIWAE model on a random hold-out set.}
